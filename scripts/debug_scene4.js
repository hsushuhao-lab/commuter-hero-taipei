...
['yu', 'shakira', 'sandra'].forEach(charId => {
  ...
  game.player.takeDamage = function(amt) {
    if (this.x > 11000) {
      console.log('[' + charId.toUpperCase() + '] HIT at x=' + Math.round(this.x) + ' amt=' + amt + ' HP_after=' + (this.hp - amt) + ' invuln=' + this.invulnerableTimer);
    }
    return origTakeDamage(amt);
  };
  ...
  console.log('[' + charId.toUpperCase() + '] Final: state=' + game.state + ' x=' + Math.round(game.player.x) + ' HP=' + game.player.hp);
});
