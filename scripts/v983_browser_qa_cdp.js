...
const outputDir = path.join(__dirname, '..', 'QA_V9_8_3');
let nextId = 1;
...
(async () => {
  ...
  if (!page) throw new Error('v9.8.3 Chrome page not found on DevTools port 9222');
  ...
  await capture(socket, 'boss_p2_dense.png');
  ...
  await capture(socket, 'victory_run_exactly_3.png');
