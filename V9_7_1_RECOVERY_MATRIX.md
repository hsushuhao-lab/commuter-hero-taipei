...
| Antigravity WIP preserved in Git checkpoint | NOT_STARTED | Branch exists but still points at `19c2296`; 43 tracked changes and 6 untracked files are dirty | Git status | Not applicable | PI decision, then reviewed recovery branch/commit/push |
...
| Hero Form 2 removed from active gameplay | PARTIAL | Active `Characters.js` and `Player.js` have no Form 2 state; stale runtime HUD/Style Bible still exposes it | `source/src/data/Characters.js:1-164`; `source/src/entities/Player.js:40-139`; `source/src/ui/HUD.js:287-297`; `source/src/ui/StyleBible.js:208` | No current v9.7.1 assertion | Remove contradictory runtime text and add a regression test |
...
| Phase 2 predator pursuit | PARTIAL | Catch-up speed, predicted target direction, re-entry, and reinforcements exist | `source/src/entities/Monster.js:143-199`; `source/src/world/Level.js:426-467` | Monster attack/stability tests pass only attack/finite-state checks | Validate persistent pursuit, front interception, flank, surround, and state synchronization |
| Phase 2 monster damage and attack density | DONE | P2 damage/cooldown variants and attack patterns are implemented | `source/src/data/Monsters.js:14-232`; `source/src/entities/Monster.js:292-584` | `monster_attack_phase_test.js` 8/8 PASS | Add population-density assertion against target range |
| Coins unchanged in placement/count | UNKNOWN | Coin declarations exist, but no independent before/after position audit was run | `source/src/world/Level.js:200-383` | Existing simulation checks item types, not unchanged placement | Compare against v9.6 baseline with an explicit manifest |
| Coffee density reduced and each coffee heals +25 | PARTIAL | 13 coffee placements in current source; +25 capped heal and no speed buff are implemented | `source/src/world/Level.js:219-383`; `source/src/entities/Player.js:154-198` | `playtest_simulation.js` assertion 4 PASS | Confirm intended baseline count and add exact density test |
...
`BLOCKED_FOR_IMPLEMENTATION_AND_RELEASE_PENDING_PI_RECOVERY_DECISION`
