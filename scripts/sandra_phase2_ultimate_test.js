...
assert(projectiles.projectiles.every(p => p.type === 'pan_wave' && p.damage === 30), 'Phase 1 Ult must deal 30 damage per pan wave');
...
assert.strictEqual(projectiles.projectiles[0].damage, 40);
phase2.updateSandraUltRelease(0.08);
assert(projectiles.projectiles.length > 1 && projectiles.projectiles.length < 14, 'Phase 2 pans must release over multiple frames');
for (let i = 0; i < 15; i++) phase2.updateSandraUltRelease(0.06);
...
assert(projectiles.projectiles.every(p => p.damage === 40), 'Phase 2 each pan must deal 40 damage');
...
assert.strictEqual(projectiles.projectiles.reduce((sum, p) => sum + p.damage, 0), 560, 'Phase 2 nominal damage must be 560');
